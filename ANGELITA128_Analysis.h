/*
    This is part of the ANGELITA128 encryption system, the source code file for the ANGELITA128 class header (Analysis version)
...
#include "ANGELITA128_Analysis_Exception.h"
#include <array>
#include <vector>
...
private:
	...
	//Analysis functions:
	std::array<unsigned char, 128> sp1_8(std::array<unsigned char, 16> bytes);
	std::array<unsigned char, 2048> sp1_8(std::array<unsigned char, 256> bytes);
	double H(std::array<unsigned char, 16> beforeBlock, std::array<unsigned char, 16> afterBlock);
	double H(std::array<unsigned char, 256> beforeBlock, std::array<unsigned char, 256> afterBlock);
	double getAverage(std::vector<double> differences);
	void setKeyA(std::array<unsigned char, 16> keyArray);
	unsigned int getPopCount(std::array<unsigned char, 16> ciphertextBlock);
...
public:
	...
	//Analysis functions
	void GeneralDifference();
	void PlaintextAvalanche();
	void KeyAvalanche();
	void SboxDifference();
	void popCountTest();
	void popCountTestBiasedPlaintext(unsigned int biasByte);
	void showKeySchedule();
	void outputKeySchedule(std::string fileName);
	void PlaintextAvalancheExamination();
